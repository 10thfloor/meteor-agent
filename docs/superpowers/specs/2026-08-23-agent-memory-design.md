...
> **Scope note (2026-08-31):** this record defines **Fact Memory** only.
> Agent-owned Experience, Practice, Constitution, and Memory Frames are a
> separate architecture; see [ADR 0001](../../adr/0001-agent-experience-memory.md).
> The “fourth quadrant” remains absent from the Fact Memory row model and is
> not a statement that Agents cannot have their own experiential continuity.
...
| 9 | **The standing block reads the collection directly** (`findAsync`, not mongot) | Immune to mongot's change-stream indexing lag. Legacy Agents rebuild it per iteration. An Agent using the Experience architecture freezes its exact Fact Memory selection in the Turn's Memory Frame instead, so a save becomes visible on the next trigger rather than changing a retry's causes. |
...
| 20 | **Children and throwaways are excluded by SESSION, not config** | Tools are built from config alone, so the suppression check reads the session document: a session carrying `parent` (subagent child) or `ephemeral` (`Agent.ask`) gets no Fact Memory tools and no Fact Memory block. Agent Identity learning is a separate later subsystem. |
