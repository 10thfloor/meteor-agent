...
| 2 | **Absent roster = classic pair; present roster = complete** | `participants` is optional and additive (the `channel`/`parent` idiom). Materialization is its own single-winner write (§4.1); the array is either absent or authoritative. "Bit-for-bit today" is scoped to *observable behavior*: `from` is stamped on every new row regardless (it is additive and invisible to old readers), and the 1:1 provider payload stays byte-identical. |
